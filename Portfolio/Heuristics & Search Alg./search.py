...
def lowestCostFirst(problem):
    ...
    while not pq.isEmpty():
        ...
        if problem.isGoalState(state) == True:
            break
        for sc in problem.getSuccessors(state):
            if not visited.has_key(sc[0]):
                new_nd = {}
                new_nd["pred"] = nd
                new_nd["state"] = sc[0]
                new_nd["act"] = sc[1]
                new_nd["cost"] = sc[2] + cost
                pq.push(new_nd, new_nd["cost"])
    ...
    while not pq.isEmpty():
        ...
        for sc in problem.getSuccessors(state):
            if not visited.has_key(sc[0]):
                ...
                pq.push(new_nd, new_nd["cost"] + new_nd["eq"])
